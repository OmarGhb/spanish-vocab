...
- **Signup → onboarding reroute (forward seam, from M6.0a).** M6.0a's `app/signup/page.tsx` does `router.push('/')` on success (correct while there's no onboarding). When the onboarding flow lands, first-run signup must route INTO onboarding, not straight to Home — reroute that single push, nothing else.
- **Real display name (M6 onboarding) — replaces the placeholder.** v0.6.7 added `displayNameFromEmail` (`lib/display-name.ts`, pure + tested): a temporary name derived from the email local-part (title-cased first token — "camille.r@…" → "Camille"), used in the drill recap ("¡Muy bien, {name}!") and now the Accueil greeting ("¡Hola, {prénom}!", M5.5k). M6 onboarding should collect a real name and store it on `profiles`; then swap **only this helper's source** (read the stored name, fall back to the email derivation) — every name surface already reads through it, so no call sites to hunt down.
...
- **Shared-components-pending immersion (make mode-aware when their LAST French consumer joins the layer).** Deliberately left French because each is shared with a not-yet-immersion surface — flipping now would leak ES into a French screen:
  - **`AudioButton`** ("Écouter la prononciation", both aria variants) — shared across dictionary/review/add/account + word-detail. Park until those join (from M6.1c).
  - **`LoadingChecklist`** ("Pendant que tu attends", L118) — shared with the still-French `/add` loader (from M6.1b).
  - ~~**`AnswerBlank`** aria-label~~ **DONE (M6.1d-i, v0.9.5)** — `AnswerBlank` now reads `useSettings` → aria via `REVIEW_CHROME.yourAnswer` (review + drill both correct).
