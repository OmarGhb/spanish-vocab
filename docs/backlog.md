...
- **`LoadingChecklist` immersion (from M6.1b).** The shared loader (`app/(app)/LoadingChecklist.tsx`) hardcodes **"Pendant que tu attends"** (L118); it stays French on Discover's generation screen in immersion/totale because the component is shared with the still-French `/add` loader. Make it mode-aware (prop or `useSettings`) when `/add` joins the layer — then Discover's loader is fully ES. Everything else on Discover is ES-complete (M6.1b).
