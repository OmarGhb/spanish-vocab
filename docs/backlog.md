...
## Drill recap — known expectation gap (NON-BUG, from v0.12.24–26 session)

- **Drill recap doesn't show "words memorized this session" — expectation gap, NOT a bug.** A tester
  expected the `/drill` end-of-session recap (`DrillRecap.tsx`) to surface newly-memorized words the way
  the `/review` bilan does (the v0.12.6 callout). It can't: the **drill is pure practice** — it writes
  **nothing** to FSRS / `review_cards` / `review_logs` (confirmed by explicit code comments in
  `DrillClient.tsx` + `lib/drill.ts`, and by grep). Its unit is a **conjugation form (verb×tense×person)**,
  not a word-card, so "memorized this session" has **no source of truth** inside the drill. The recap shows
  score `correct/total`, a % bar, and the missed list (verb · tense · person, wrong→correct) — no mastery
  concept. Matching the review bilan would mean cross-referencing drilled verbs against `review_cards`
