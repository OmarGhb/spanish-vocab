...
- **Cue selection is FIXED per card, not rotating** (`pickClozeExample`): `seed` derives from the card UUID, so an infinitive-stored verb shows the **same** form every review. Deliberate (deterministic SSR/hydration); per-review rotation would need a review-varying seed input — not done.
...
- **Two review-format items scoped to v0.6.3 ("écriture-polish"), NOT open backlog** (see roadmap): (1) **clitic-aware masking** so clitic-stored reflexives (`"te duermes"`) get a coherent cloze instead of routing to definition-MCQ; (2) **`maskVerbSentence` accent-homograph false positive** — `dé→de`, `sé→se`, `él→el`, `té→te` accent-fold to function words, so the verb path can mask a preposition/article instead of the verb (confirmed: `dar` blanks `"de"`). Affects **écriture too** (no cue-gate there → écriture would test the function word), so it's a real `maskVerbSentence` fix, not data hygiene.
