...
**Conjugation-table half ✅ (v0.6.6)** · Astuce-de-Paco /review hint **NOT YET BUILT** (separate slice).
...
- **Astuce-de-Paco /review hint** (the ④ post-answer morphology tip) — NOT YET BUILT; mounts the same `ConjugationGrid` component. May add the persisted `examples[].target = {lemma,tense,person}` field at enrich if the Astuce actually needs it (deferred from M5.3a).
