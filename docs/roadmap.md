...
- **Known limit:** `te sientas` stays definition-MCQ — the conjugator lacks `sentar`'s e→ie present stem (`sientas`); auto-upgrades when that's added. See the M5.3c conjugator-hardening gate below.
...
- **⛔ GATE — M5.3c is blocked on conjugator hardening.** Through v0.6.3 the conjugator's correctness has been **non-load-bearing**: nothing displays a generated form (the #2 denylist *skips* uncertain tokens, masking only *locates* the real Anthropic-authored token). M5.3c makes generated forms **user-facing** (the drill prompts + conjugator-generated wrong-FORM distractors), so a wrong form would teach the learner wrong. Before any generated form becomes user-facing, M5.3c must: (1) **extend the M5.3a/b "refuse to display untabled/uncertain forms" guard** to the drill + distractors; AND (2) **fix the tabled irregulars** — stem-changers and imperatives — surfaced as the leading edge of a conjugator audit (see `backlog.md`): unaccented usted-imperatives (`dar→de`, `estar→esta` vs `dé/está`) and the missing `sentar` e→ie present stem (`sientas`).
