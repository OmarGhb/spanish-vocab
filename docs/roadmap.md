...
### M5.5g+ — remaining per-screen cluster reworks (not yet scoped)
- Home cluster. The Astuce + conjugation-table review interstitial (M5.3b, deferred — its own milestone). Each consumes the M5.5a substrate. **Final step:** signup/login/onboarding (Claude Design first) → M6 build (onboarding bumped from M5.5f to **M5.5g**).
