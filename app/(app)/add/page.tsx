...
export default function AddPage() {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === 'SPELLCHECK_CANDIDATES' && data.candidates) {
          ...
          setPhase({ tag: 'idle' })
          setInputError(resolveChrome(ADD_CHROME.notInSpanish, ctx))
        } else {
  ...
  if (phase.tag === 'idle') {
    return (
      <div className="flex flex-col pb-16">
        <div className="p-5">
          <h1 className="font-serif text-[30px] font-bold tracking-[-0.02em] text-ink">{resolveChrome(ADD_CHROME.newWord, ctx)}</h1>
          <p className="text-[13.5px] text-muted mt-1">{resolveChrome(ADD_CHROME.enterWord, ctx)}</p>
        </div>
        ...
        <div className="px-5 flex flex-col gap-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-[14px] text-muted leading-relaxed">
              {resolveChrome(ADD_CHROME.helperParagraph, ctx)}
            </p>
          </div>
          <Button
            variant="primary"
          ...
          >
            {resolveChrome(ADD_CHROME.search, ctx)} →
          </Button>
        </div>
      </div>
    )
  ...
  if (phase.tag === 'spellcheck_candidates') {
    ...
    return (
      <div className="flex flex-col p-5 gap-5 pb-16">
        ...
        >
          ← {resolveChrome(DRILL_CHROME.back, ctx)}
        </button>
        <div>
          <h1 className="font-serif text-[28px] font-bold tracking-[-0.02em] text-ink">{resolveChrome(ADD_CHROME.didYouMean, ctx)}</h1>
        ...
        >
          {resolveChrome(ADD_CHROME.noneOfThese, ctx)}
        </button>
      </div>
    )
  }
  ...
  if (phase.tag === 'lemma_suggestion') {
    ...
    return (
      <div className="flex flex-col p-5 gap-5 pb-16">
        ...
        <div>
          <h1 className="font-serif text-[28px] font-bold tracking-[-0.02em] text-ink">{resolveChrome(ADD_CHROME.conjugatedForm, ctx)}</h1>
          <p className="text-[13.5px] text-muted mt-1.5">
            {ctx.policy === 'visible'
        ...
        {grid ? (
          ...
          result.form_annotation && (
            <div className="bg-card border border-line rounded-[14px] shadow-card px-4 py-[14px]">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted">{resolveChrome(DETAIL_CHROME.formEyebrow, ctx)}</p>
        ...
        {lemma_status === 'already_in_deck' && (
          <p className="flex items-center gap-2 font-serif text-[13.5px] italic text-muted leading-relaxed">
            ...
            <span>
              ...
              {dueDays !== null &&
                (dueDays <= 0
                  ? resolveChrome(ADD_CHROME.nextReviewToday, ctx)
                  : ctx.policy === 'visible'
        ...
        <div className="flex flex-col gap-3 items-center">
          ...
          >
            {saveState === 'saving' ? (
              ...
              resolveChrome(ADD_CHROME.errorRetry, ctx)
            ) : (
              <><Plus size={17} strokeWidth={2.2} /> {ctx.policy === 'visible' ? `Ajouter « ${lemma} »` : `Añadir «${lemma}»`}</>
            )}
          </Button>
          {lemma_status === 'already_in_deck' && (
            <Button
            ...
            >
              <ExternalLink size={16} /> {ctx.policy === 'visible' ? `Ouvrir « ${lemma} »` : `Abrir «${lemma}»`}
            </Button>
          )}
          <Button
            variant="text"
          ...
          >
            {ctx.policy === 'visible' ? `Garder « ${result.word} »` : `Conservar «${result.word}»`}
          </Button>
        </div>
      </div>
    )
  ...
  if (phase.tag === 'exists') {
    ...
    const supportive = dueNow
      ? resolveChrome(ADD_CHROME.noNeedRecreateReview, ctx)
      : ctx.policy === 'visible'
      ...
      <div className="flex flex-col flex-1 p-5">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-[14px] text-muted leading-relaxed mt-2 max-w-[260px]">
            {resolveChrome(ADD_CHROME.bringTomorrow, ctx)}
          </p>
          <div className="w-full max-w-[320px] mt-[22px]">
            <WordRow id={id || undefined} word={w} defEs={defEs} card={null} asListItem={false} ctx={ctx} />
          </div>
          <div className="w-full max-w-[320px] mt-[22px] flex flex-col gap-3 items-center">
            <Button variant="primary" full type="button" onClick={handleAddAnother}>
              <Plus size={17} strokeWidth={2.2} /> {resolveChrome(ADD_CHROME.addAnother, ctx)}
            </Button>
            <Button variant="text" href="/words">
              {resolveChrome(ADD_CHROME.viewMyWords, ctx)}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 p-5">
        <div className="flex flex-col items-center text-center pt-3">
          ...
          <p className="text-[14px] text-muted leading-relaxed mt-2 max-w-[290px]">
            {resolveChrome(ADD_CHROME.wordsEnRouteHelp, ctx)}
          </p>
        </div>
        <div className="w-full max-w-[330px] mx-auto mt-5 flex flex-col gap-2.5">
          ...
          {words.map((w) => (
            <WordRow key={w} word={w} defEs="" card={null} asListItem={false} ctx={ctx} />
          ))}
        </div>
      ...
      <div className="px-5 flex flex-col gap-4 pb-4">
        ...
        {result.form_annotation && (
          <div className="bg-card border border-line rounded-[14px] shadow-card p-5">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted mb-3">{resolveChrome(DETAIL_CHROME.formEyebrow, ctx)}</p>
        ...
        <div className="bg-card border border-line rounded-[14px] shadow-card p-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted mb-3">{resolveChrome(DETAIL_CHROME.definitionEyebrow, ctx)}</p>
          ...
          {gloss !== 'hidden' && (revealedDefFr ? (
            <div className="mt-2">
              ...
              <button type="button" onClick={() => setRevealedDefFr(false)} className="text-[13px] font-semibold text-accent underline underline-offset-[3px] mt-1.5">
                {resolveChrome(DETAIL_CHROME.hideDef, ctx)}
              </button>
            </div>
          ) : (
            <button type="button" onClick={() => setRevealedDefFr(true)} className="text-[13px] font-semibold text-accent underline underline-offset-[3px] mt-2">
              {resolveChrome(DETAIL_CHROME.revealDef, ctx)}
            </button>
          ))}
        </div>
        ...
        {/* EXEMPLES */}
        <div className="bg-card border border-line rounded-[14px] shadow-card p-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted mb-4">{resolveChrome(DETAIL_CHROME.examplesEyebrow, ctx)}</p>
          ...
          <div className="bg-surface-alt border border-tinted-border rounded-[14px] p-5">
            <div className="flex justify-between items-center mb-2">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted">
                {resolveChrome(ADD_CHROME.similarNotConfuse, ctx)}
              </p>
              <button
                type="button"
              ...
              >
                {resolveChrome(allSelected ? ADD_CHROME.deselectAll : ADD_CHROME.selectAll, ctx)}
              </button>
            </div>

            <p className="text-[12.5px] text-muted leading-relaxed mb-3">
              {resolveChrome(ADD_CHROME.similarHelper, ctx)}
            </p>
        ...
        {selectionCount > 0 ? (
          ...
          <>
            <button
              type="button"
              onClick={handleAddAnother}
              aria-label={resolveChrome(ADD_CHROME.newWord, ctx)}
            ...
            >
              {saveState === 'saving' ? (
                ...
                <><Plus size={17} strokeWidth={2.2} /> {resolveChrome(ADD_CHROME.addToCollection, ctx)}</>
              )}
            </Button>
          </>
        )}
