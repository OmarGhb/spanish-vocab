...
import { resolveChrome, DRILL_CHROME, REVIEW_CHROME, type ChromePair, type ImmersionMode } from '@/lib/immersion'
...
const META: Record<DrillVerdict, { img: string; excl: string; color: string; tone: 'accent' | 'ok' | 'err' }> = {
  correct: { img: '/paco-feliz.png', excl: '¡Eso es!', color: 'text-ok', tone: 'ok' },
  close: { img: '/paco-pensando.png', excl: '¡Casi!', color: 'text-ink', tone: 'accent' },
  wrong: { img: '/paco-sad.png', excl: '¡Uy!', color: 'text-err', tone: 'err' },
}
// Verdict note — mode-aware (reuses Review's "du premier coup"; drill-specific presque/pas tout à fait).
const NOTE_PAIR: Record<DrillVerdict, ChromePair> = {
  correct: REVIEW_CHROME.noteFirstTry,
  close: DRILL_CHROME.almost,
  wrong: DRILL_CHROME.notQuite,
}
...
export default function DrillResult({
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      <DrillHeader count={count} total={total} tenseLabel={tenseLabel(prompt.tense)} tone={m.tone} onExit={onExit} mode={mode} />
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pt-6">
        ...
        <div className="fade-up flex items-end gap-3.5">
          ...
          <div className="pb-1.5">
            ...
            <p className="mt-1.5 text-[13.5px] text-muted">{resolveChrome(NOTE_PAIR[verdict], mode)}</p>
          </div>
        </div>
        ...
        <div className="fade-up mt-6 bg-card border border-line rounded-card p-5" style={{ animationDelay: '0.1s' }}>
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted">
            {resolveChrome(verdict === 'correct' ? DRILL_CHROME.theAnswer : DRILL_CHROME.wasAnswer, mode)}
          </p>
          ...
          {verdict !== 'correct' && (
            <p className="mt-2 font-serif text-[14.5px] italic text-muted">
              {resolveChrome(REVIEW_CHROME.yourAnswer, mode)} :{' '}
        ...
        {grid && (
          <div className="fade-up mt-4 pb-2" style={{ animationDelay: '0.24s' }}>
            <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-muted">{resolveChrome(DRILL_CHROME.theConjugation, mode)}</p>
      ...
      <div className="px-5 pt-3.5 shrink-0 pb-[max(1.75rem,env(safe-area-inset-bottom))]">
        ...
        <Button variant="primary" full type="button" onClick={onNext}>
          {resolveChrome(isLast ? DRILL_CHROME.seeSummary : DRILL_CHROME.next, mode)}
