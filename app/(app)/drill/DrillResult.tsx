...
import { resolveChrome, DRILL_CHROME, REVIEW_CHROME, type ChromePair, type ChromeCtx } from '@/lib/immersion'
...
export default function DrillResult({
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      <DrillHeader count={count} total={total} tenseLabel={tenseLabel(prompt.tense)} tone={m.tone} onExit={onExit} ctx={ctx} />
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pt-6">
        ...
        <div className="fade-up flex items-end gap-3.5">
          ...
          <div className="pb-1.5">
            ...
            <p className="mt-1.5 text-[13.5px] text-muted">{resolveChrome(NOTE_PAIR[verdict], ctx)}</p>
          </div>
        </div>
        ...
        <div className="fade-up mt-6 bg-card border border-line rounded-card p-5" style={{ animationDelay: '0.1s' }}>
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted">
            {resolveChrome(verdict === 'correct' ? DRILL_CHROME.theAnswer : DRILL_CHROME.wasAnswer, ctx)}
          </p>
          ...
          {verdict !== 'correct' && (
            <p className="mt-2 font-serif text-[14.5px] italic text-muted">
              {resolveChrome(REVIEW_CHROME.yourAnswer, ctx)} :{' '}
        ...
        {grid && (
          <div className="fade-up mt-4 pb-2" style={{ animationDelay: '0.24s' }}>
            <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-muted">{resolveChrome(DRILL_CHROME.theConjugation, ctx)}</p>
      ...
      <div className="px-5 pt-3.5 shrink-0 pb-[max(1.75rem,env(safe-area-inset-bottom))]">
        ...
        <Button variant="primary" full type="button" onClick={onNext}>
          {resolveChrome(isLast ? DRILL_CHROME.seeSummary : DRILL_CHROME.next, ctx)}
