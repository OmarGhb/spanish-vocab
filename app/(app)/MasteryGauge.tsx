...
import { DEFAULT_CHROME_CTX, type ChromeCtx } from '@/lib/immersion'
...
export default function MasteryGauge({ card, ctx = DEFAULT_CHROME_CTX }: { card: WordCard | null; ctx?: ChromeCtx }) {
  const level = getMasteryGauge(card)
  const aria = ctx.policy === 'visible' ? `Mémorisation : ${level} / 4` : `Memorización : ${level} / 4`
  return (
