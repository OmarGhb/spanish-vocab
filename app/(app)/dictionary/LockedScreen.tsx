...
import { DEFAULT_CHROME_CTX, resolveChrome, DICT_CHROME, type ChromeCtx } from '@/lib/immersion'
...
export default function LockedScreen({ memorizedCount, ctx = DEFAULT_CHROME_CTX }: { memorizedCount: number; ctx?: ChromeCtx }) {
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center px-9 pb-14 text-center">
      ...
      <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-accent mt-4">{resolveChrome(DICT_CHROME.personalLexicon, ctx)}</p>
      <h1 className="font-serif text-[27px] font-bold text-ink leading-[1.12] tracking-[-0.02em] mt-2">
        {resolveChrome(DICT_CHROME.dictWaits, ctx)}
      </h1>
      <p className="text-[14.5px] text-muted leading-relaxed max-w-[290px] mt-3">
        {resolveChrome(DICT_CHROME.lockedBody, ctx)}
      </p>
      ...
      <div className="w-full max-w-[300px] mt-[30px] bg-card border border-line rounded-2xl shadow-card px-[18px] pt-[18px] pb-5">
        <div className="flex items-baseline justify-between mb-2.5">
          <span className="text-[13.5px] font-bold tracking-[0.02em] text-muted tabular-nums">
            {x} / {DICTIONARY_UNLOCK_THRESHOLD} {ctx.policy === 'visible' ? 'mémorisés' : 'memorizadas'}
          </span>
